...
// Token contracts hook
export function useTokenContracts() {
  const connection = useConnection();

  // Token contracts
  const ethierTokenAbi = [
    {
      inputs: [
        {
  ...
  const ethierTokenContract = new connection.eth.Contract(
    ethierTokenAbi as any,
    "0x3ADEEec2A26FFf303402325Faa770F56D1114174"
  );

  return {
    ETHI: ethierTokenContract,
  };
}
